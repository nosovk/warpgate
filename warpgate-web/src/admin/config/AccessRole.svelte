<script lang="ts">
  ...
  async function loadUsers(): Promise<PaginatedResponse<User>> {
      const targets = await api.getRoleUsers({
          id: params.id,
      })
      return {
          items: targets,
  ...
  async function loadTargets(): Promise<PaginatedResponse<Target>> {
      const targets = await api.getRoleTargets({
          id: params.id,
      })
      return {
          items: targets,
...
<div class="container-max-md">
  ...
  <div class="d-flex">
    ...
    <AsyncButton
      color="primary"
      disabled={!adminPermissions.value.accessRolesEdit}
      class="ms-auto"
      click={update}>Update</AsyncButton
    >

    <AsyncButton
      class="ms-2"
      disabled={!adminPermissions.value.accessRolesDelete}
      color="danger"
      click={remove}>Remove</AsyncButton
    >
  </div>
