...
<script lang="ts">
    ...
    async function loadRoles() {
        ...
        roleIsAllowed = Object.fromEntries(
            allowedRoles.map((r) => [r.id, true]),
        )
        return allRoles
    }
...
<div class="container-max-md">
    ...
    {#if target}
        ...
        >
            ...
            <ModalBody>
                ...
                {#key connectionsInstructionsModalOpen}
                    <!-- regenerate examples when modal opens -->
                    <ConnectionInstructions
                        targetName={target.name}
                        username={selectedUsername}
                        targetKind={target.options.kind}
                        targetExternalHost={target.options.kind ===
                        TargetKind.Http
                            ? target.options.externalHost
                            : undefined}
            ...
            <ModalFooter>
                <Button
                    ...
                    block
                    on:click={() =>
                        (connectionsInstructionsModalOpen = false)}
                >
                    Close
                </Button>
            </ModalFooter>
        ...
        {#if target.options.kind === 'Postgres'}
            <FormGroup floating label="Idle timeout">
                ...
                <small class="form-text text-muted">
                    How long an authenticated session can remain idle before
                    requiring re-authentication. Examples: 30m, 1h, 2h30m.
                    Leave empty for default (10m).
                </small>
            </FormGroup>
        {/if}
        ...
        {#if target.options.kind === 'MySql' || target.options.kind === 'Postgres'}
            ...
            >
                ...
                />
                <small class="form-text text-muted">
                    Default database name used in connection examples. This
                    is only for display purposes and does not restrict which
                    databases users can access. Leave empty to use the
                    global default.
                </small>
            </FormGroup>
        {/if}
    ...
    <AsyncButton
        ...
        color="danger"
        click={remove}
        disabled={!$adminPermissions.targetsDelete}>Remove</AsyncButton
    >
    </div>
</div>
