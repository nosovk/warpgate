...
<div class="container-max-md">
    ...
    {#if target}
          ...
          {#key connectionsInstructionsModalOpen}
            ...
            <ConnectionInstructions
              ...
              targetDefaultDatabaseName={target.options.kind ===
                TargetKind.MySql || target.options.kind === TargetKind.Postgres
      ...
      {#if target.options.kind === 'Http'}
        ...
        {#if serverInfo.value?.externalHost}
          <FormGroup floating label="Bind to a domain">
            <Input
              type="text"
              placeholder={'foo.' + serverInfo.value.externalHost}
      ...
      {#if target.options.kind === 'Postgres'}
        <FormGroup floating label="Idle timeout">
          ...
          <small class="form-text text-muted">
            How long an authenticated session can remain idle before requiring
            re-authentication. Examples: 30m, 1h, 2h30m. Leave empty for default
            (10m).
          </small>
        </FormGroup>
      {/if}
      ...
      {#if target.options.kind === 'MySql' || target.options.kind === 'Postgres'}
        ...
        >
          ...
          />
          <small class="form-text text-muted">
            Default database name used in connection examples. This is only for
            display purposes and does not restrict which databases users can
  ...
  <div class="d-flex">
    ...
    <AsyncButton
      ...
      color="danger"
      click={remove}
      disabled={!adminPermissions.value.targetsDelete}>Remove</AsyncButton
    >
  </div>
</div>
