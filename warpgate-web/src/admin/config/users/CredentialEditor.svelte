...
<script lang="ts">
    ...
    const policyProtocols: {
        id: 'ssh' | 'http' | 'mysql' | 'postgres' | 'kubernetes'
        name: string
    }[] = [
        { id: 'ssh', name: 'SSH' },
        { id: 'http', name: 'HTTP' },
...
<div class="d-flex align-items-center mt-4 mb-2">
    ...
    <Button
        size="sm"
        color="link"
        on:click={() => (creatingPassword = true)}
    >
        Add password
    </Button>
    <Button
...
<Loadable promise={loadPromise}>
    ...
    {#each credentials as credential (credential.id)}
        <div class="list-group-item credential gap-2">
            ...
            {#if credential.kind === CredentialKind.Certificate}
                ...
                <div class="main me-auto abbreviate">
                    ...
                    <small class="d-block text-muted abbreviate"
                        >SHA-256: <code>{credential.fingerprint}</code
                        ></small
                    >
                </div>
                <CredentialUsedStateBadge {credential} />
            ...
            {#if credential.kind === CredentialKind.PublicKey || credential.kind === CredentialKind.Sso}
                <Button
                    ...
                    disabled={credential.kind ===
                        CredentialKind.PublicKey &&
                        (ldapLinked || !$adminPermissions.usersEdit)}
                    onclick={(e) => {
                        if (credential.kind === CredentialKind.Sso) {
            ...
            <Button
                class="px-0"
                color="link"
                disabled={credential.kind === CredentialKind.PublicKey &&
                    (ldapLinked || !$adminPermissions.usersEdit)}
                onclick={(e) => {
    ...
    {#each policyProtocols as protocol (protocol)}
        {@const effectiveCredentials = getEffectivePossibleCredentials(
            protocol.id,
        )}
