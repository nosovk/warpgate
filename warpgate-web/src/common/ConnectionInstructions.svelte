...
{#if targetKind === TargetKind.Postgres}
    ...
    <input
        ...
        class="form-control"
        readonly
        value={examplePostgreSQLCommand}
    />
                ...
                <ListGroup flush class="mb-2">
                    {#each certificates as cert (cert.credential.id)}
                        ...
                        >
                              ...
                              {#if cert.hasLocalKey}
                                  <Badge
                                      id="cert-status-badge-{cert.credential
                                          .id}"
                                      color="success">Key available</Badge
                                  >
                              {:else}
                                  <Badge
                                      id="cert-status-badge-{cert.credential
                                          .id}"
                                      color="warning">No private key</Badge
                                  >
                              {/if}
                              <Tooltip
                                  target={'cert-status-badge-' +
                                      cert.credential.id}
                                  placement="top"
                                  delay={500}
                              >
                                  {#if cert.hasLocalKey}
                                      This certificate's private key is stored
                                      locally in this browser and can be used
                                      to generate a kubeconfig with working
                                      credentials.
                                  {:else}
                                      This certificate's private key is not
                                      stored locally in this browser. You can
                                      still generate a kubeconfig, but it will
                                      contain placeholders for authentication
                                      and won't work until you fill in the
                                      actual certificate and key data.
                                  {/if}
                              </Tooltip>
                        </ListGroupItem>
                    {/each}
                ...
                {#if !selectedCertId || !clientPrivateKeyPem}
                    <InfoBox class="mb-2">
                        {#if certificates.length > 0}
                              ...
                              {:else}
                                  The private key for this certificate is not
                                  stored in this browser.
                              {/if}
                              The kubeconfig will contain placeholders for authentication.
                        {:else}
                              You need a certificate credential to connect to
                              this target.
                        {/if}
                    </InfoBox>
                    <InfoBox>
                        You can issue a new certificate using the button
                        above and enable the "Store in browser" option to
                        generate a ready-to-use kubeconfig.
                    </InfoBox>
                {/if}
            {/if}
        {/if}
    ...
    <div class="col-12 col-lg-6">
        ...
        >
            <textarea
                class="form-control"
                readonly
                style="height: 27rem; font-family: monospace; font-size: 0.9em;"
                >{kubeconfig}</textarea
            >
        ...
        <InfoBox class="mt-3">
            Save the kubeconfig above to a file (e.g. <code
                >warpgate-kubeconfig.yaml</code
            >) and use it with kubectl.
        </InfoBox>
    </div>
    </div>
{/if}
